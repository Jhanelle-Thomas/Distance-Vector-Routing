...
class LearningSwitch(api.Entity):
    ...
    def handle_link_down(self, port):
        ...
        """
        for dest in self.fwd_table:
			if self.fwd_table[dest] == port:
				del self.fwd_table[dest]
    ...
    def handle_rx(self, packet, in_port):
        ...
        if isinstance(packet, basics.HostDiscoveryPacket):
			...
			self.fwd_table[source] = in_port
            return		
		if source not in self.fwd_table:
			self.fwd_table[source] = in_port

        if destination not in  self.fwd_table:
			# Flood out all ports except the input port
			self.send(packet, in_port, flood=True)
		else:
			self.send(packet, self.fwd_table[destination])
